...
public class SerialUtilOld {
    ...
    private  static SerialUtilOld serialUtilOld = null;
    private SerialUtilOld(){};
    public static SerialUtilOld serialUtilOld (){
        if(serialUtilOld == null){
            serialUtilOld = new SerialUtilOld();
        }
        return serialUtilOld;
    }
     ...
     *  传入数据    得到数据
     ...
     public static  byte[] changeData(byte[] bytes) {
         byte[] data = {};
         if(bytes != null && bytes.length != 0){
             serialUtilOld.setData(bytes);
             data = serialUtilOld.getData();
             if(data.length != 0){
                  return data;
             }
         }
